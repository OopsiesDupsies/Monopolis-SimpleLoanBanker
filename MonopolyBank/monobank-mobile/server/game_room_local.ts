...
// ===== MAIN CLASS (adapted for Node.js - no Cloudflare) =====
...
export class GameRoom {
    ...
    constructor(roomCode: string, roomName?: string, isPrivate?: boolean, password?: string) {
        this.sessions = [];
    ...
    getRoomInfo(): any {
        return {
            ...
            lastActivityAt: this.gameState.lastActivityAt || this.createdAt,
    ...
    handleSession(ws: WebSocket): void {
        ...
        ws.on("close", () => {
            ...
            if (this.sessions.length === 0 && this.gameState.players.length === 0) {
                console.log(`Room ${this.gameState.roomCode} is empty, ready for cleanup`);
            }
        });
    }

    // No-op save (in-memory only, no persistent storage needed for local WiFi)
