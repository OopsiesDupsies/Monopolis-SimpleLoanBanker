...
const WS_API = import.meta.env.VITE_WS_URL ? `${import.meta.env.VITE_WS_URL}/api` : 'wss://monopoly-bank-worker.oopsoops-ty.workers.dev/api';
const HTTP_API = import.meta.env.VITE_API_URL ? `${import.meta.env.VITE_API_URL}/api` : 'https://monopoly-bank-worker.oopsoops-ty.workers.dev/api';
...
export default function GameRoom() {
    ...
    const shouldReconnect = useRef(true); // Controls whether we should auto-reconnect

    const connectWebSocket = useCallback(() => {
        if (!code || !shouldReconnect.current) return;

        const password = location.state?.password;
        const wsUrl = `${WS_API}/rooms/${code}/websocket${password ? `?password=${encodeURIComponent(password)}` : ''}`;
        ...
        socket.onmessage = (event) => {
            ...
            if (msg.type === 'state') {
                ...
                // Show error as a temporary toast, don't block the UI
        ...
        socket.onclose = () => {
            ...
            setConnected(false);
            // Auto-reconnect unless intentionally left/kicked
        ...
        socket.onerror = () => {
            ...
            // Don't set a blocking error — reconnect will handle it
        };
    }, [code, location.state]);
    ...
    useEffect(() => {
        ...
        // First check if room exists, then connect
        const checkAndConnect = async () => {
            try {
                const res = await fetch(`${HTTP_API}/rooms/${code}`);
                if (!res.ok) {
    ...
    if (!me) {
        return (
            <div className="app-container" style={{ padding: 20, display: 'flex', flexDirection: 'column', justifyContent: 'center' }}>
                <h1 className="text-center mb-4">Join Room {code}</h1>
                {error && (
                    <div style={{
                        padding: 16,
    ...
    if (state.status === 'lobby') {
        return (
            <>
                ...
                <LobbyView state={state} me={me} send={send} roomCode={code || ''} isPrivate={state.isPrivate} />
            </>
        );
    }
