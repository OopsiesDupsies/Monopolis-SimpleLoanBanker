...
export default function LobbyView({ state, me, send, roomCode, isPrivate }: LobbyViewProps) {
    const isHost = me?.isHost;
    const [settings, setSettings] = useState(state.settings);
    const [editingName, setEditingName] = useState(false);
    const [roomName, setRoomName] = useState(state.roomName);

    // Use isPrivate to show a badge
    const privacyBadge = isPrivate ? (
        <span style={{
            ...
            alignItems: 'center',
            gap: 6,
            border: '1px solid rgba(244, 199, 82, 0.2)'
        }}>
    ...
    return (
        <div className="app-container" style={{ padding: 24, maxWidth: 600, margin: '0 auto' }}>
            <header className="mb-6" style={{ textAlign: 'center' }}>
                ...
                {privacyBadge && <div style={{ marginBottom: 12 }}>{privacyBadge}</div>}
                ...
                <div style={{
                    ...
                    <span style={{ fontSize: 13, color: 'var(--text-muted)', textTransform: 'uppercase', letterSpacing: 1.5, marginRight: 8 }}>Room Code</span>
                    <span style={{ fontSize: 18, fontFamily: 'monospace', fontWeight: 700, color: 'var(--text-primary)', letterSpacing: 2 }}>{roomCode}</span>
                </div>
            </header>
